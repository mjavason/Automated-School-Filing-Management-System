...
if (!isset($_GET['upload_level'])) {
    $year = "year" . $_SESSION['student_next_filing_session'];

    // array_push($array, ['error' => 'Some files are missing(upload level included). Make sure all are selected and uploaded.']);
    // echo (json_encode($array));
    // die;
}else{
    $year = "year" . $_GET['upload_level'];
}
...
if (is_dir($targetPath1) && is_dir($targetPath2) && is_dir($targetPath3) && is_dir($targetPath4) && is_dir($targetPath5)) {
    ...
    if (isset($_GET['upload_id'])) {
        ...
        if (updateFileUploads($_GET['upload_id'], $file1, $file2, $file3, $file4, $file5)) {
            array_push($array, ['success' => 'Files Successfully Updated!']);
            echo (json_encode($array));
        } else {
            array_push($array, ['error' => 'Not Updated. Unknown DB error occured']);
            echo (json_encode($array));
        }
    } else {
        if (addNewFileUploads($file1, $file2, $file3, $file4, $file5)) {
            ...
            array_push($array, ['error' => 'Not Saved. Unknown DB error occured']);
            echo (json_encode($array));
        }
    }
} else {
